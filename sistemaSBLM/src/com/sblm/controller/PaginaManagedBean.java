...
public class PaginaManagedBean implements Serializable {
	...
	public void registrarPagina() {
		...
		if (!actualizado) {
			...
			Pagina pag = new Pagina();
			pag = getPaginaService().verificarPaginaEnModulo(paginamodulo.getPagina().getDescripcionpagina(),paginamodulo.getPagina().getNombrepagina(),paginamodulo.getModulo().getIdmodulo());
			if (pag==null) {
