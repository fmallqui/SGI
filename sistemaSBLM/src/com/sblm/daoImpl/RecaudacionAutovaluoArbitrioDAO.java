...
//		usr.setArea(area);
	...
	public List<Contrato> listaContratos() {
		Session session = getSessionFactory().openSession();
		return session.createQuery("select C from Contrato C inner join C.upa U inner join C.inquilino I inner join U.inmueble I inner join U.uso inner join U.ubigeo UB").list();
	}
